...
Contains data that must be available to the Perl libraries at run time,
in this case the browser code for viewing ITS metadata that needs to be
copied into project sub-folders when generating HTML output.
